...
В конце каждой сессии предлагать экспорт:
- **Markdown** — для человеческого чтения
- **JSON** — для данных и миграции
...
## Stage 5: Google Calendar Integration

### Когда использовать
- Пользователь просит "синхронизируй с календарём"
- Создание recurring событий (Weekly Review, WOOP)
- Блокировка времени на важные задачи
- Создание daily top-3 как задач в Google Tasks

### Возможности
| Функция | Описание |
|---------|----------|
| `get_events()` | Чтение встреч на период |
| `create_weekly_review_reminder()` | Напоминание о ретроспективе (каждое воскресенье) |
| `create_woop_reminder()` | WOOP-сессия (будни утром) |
| `create_milestone_event()` | Milestone 12-Week Year с напоминанием |
| `create_time_block()` | Блок времени для deep work |
| `create_daily_top3()` | 3 приоритета дня в Google Tasks |
| `get_free_slots()` | Поиск свободных слотов |

### Безопасность
- OAuth 2.0 с автоматическим refresh
- Fernet-шифрование для токенов (AES-128)
- Хранение salt в отдельном файле (0o600)
- Encryption key обязателен — нет insecure fallback

### Требования
- Python 3.9+, `google-api-python-client`, `google-auth-oauthlib`
- `credentials.json` из Google Cloud Console
- `CALENDAR_ENCRYPTION_KEY` переменная окружения
...
- Calendar: OAuth2 + Fernet encryption + 6 presets (weekly review, WOOP, milestones, tasks)
