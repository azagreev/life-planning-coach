...
**Backfill при mid-session подключении Google Drive**: предложи «У тебя накопилось данных за сессию — синхронизировать в Google Drive?» → bootstrap wiki + dump state v2. См. `references/state_v2_schema.md` и шаблоны wiki (`references/templates/AI_Instructions.md`, `references/templates/Hot_Cache.md`, `references/templates/Goals.md`, `references/templates/Wheel_of_Life_History.md`, `references/templates/Raw_Session.md`).
...
| Google Google Drive недоступен | Graceful fallback на Claude Memory + Paper Coach. |
...
- **Google Google Drive**: Данные в `Life Planning Coach Wiki/`. Скилл обновляет файлы, не имеет прямого доступа к токенам.
