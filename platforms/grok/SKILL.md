...
- **Уровень 3**: Grok Skills (forthcoming) — reusable скиллы через slash commands. Leaked UI: Name/Description/Instruction fields + .md import.
...
- **Skills System** (forthcoming): Reusable скиллы через slash commands (`/<skill-name>`). Leaked UI mockups показывают Name/Description/Instruction fields и импорт .md/.zip/.skill. Feature flag пока False — может быть недоступен для всех пользователей.
