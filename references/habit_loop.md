...
**Tiny Habits Recipe:**
```
После того как я [СУЩЕСТВУЮЩАЯ ПРИВЫЧКА],
я буду [НОВАЯ КРОШЕЧНАЯ ПРИВЫЧКА].
```
...
```
После утреннего кофе         →  я напишу 1 предложение в дневник
После обеда                  →  я сделаю 10 приседаний
После закрытия ноутбука      →  я подготовлю одежду на завтра
После чистки зубов вечером   →  я прочитаю 1 страницу
```
...
| High Friction (плохо) | Low Friction (хорошо) |
|----------------------|----------------------|
| Спортзал в 30 минутах | Коврик в комнате |
| Готовить с нуля | Заранее порезанные овощи |
| Читать книгу с полки | Книга на подушке |
...
```
Дни 1–21:   Осознанное усилие (префронтальная кора активна)
Дни 18–66:  Переход (усилие снижается, но ещё не автоматизм)
Дни 66+:    Плато автоматичности (базальные ганглии ведут)
```
...
```
Goal (BHAG/OKR) → Weekly Priority → WOOP Action → Habit Loop → Daily Execution
```
...
| Feature | Connection |
|---------|-----------|
| **WOOP** | if-then plans = Implementation Intentions = habit triggers |
...
| WOOP | Habit Loop |
|------|-----------|
| Одноразовые / редкие задачи | Ежедневные / регулярные действия |
| Сложные препятствия | Простое повторение |
| Требует ментальной контрастности | Требует автоматичности |
...
```
□ Меньше 30 секунд? (Tiny Habits)
□ Привязана к существующей рутине? (Anchor / Habit Stacking)
□ Стабильный контекст? (Wood)
□ Трение минимально? (Environment design)
□ Есть моментальное празднование? (Fogg)
□ Соответствует идентичности? (Clear)
□ Реалистичные ожидания: 2+ месяца? (Lally)
□ Пропуск = данные, не провал? (Recovery Protocol)
```
...
|---------|--------|-------------|
| Habit Loop | Duhigg (2012) | Cue-Routine-Reward; basal ganglia storage |
| Context-Dependent Repetition | Wood & Neal (2007) | 43% daily behavior is habitual; context > willpower |
| Tiny Habits | Fogg (2019) | B = MAP; celebration > repetition count |
| Habit Stacking | Clear (2018) | 64% higher adoption with anchor habits |
| Habit Timeline | Lally et al. (2010) | Median 66 days (range 18–254); 21-day rule is myth |
