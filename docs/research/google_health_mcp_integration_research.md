...
## Addendum (2026-06-04): Whoop — официальный API через агрегатор; reverse-eng BLE отклонён

**Контекст.** Поступил запрос «применить к проекту `johnmiddleton12/my-whoop` + Reddit-тред reverse-engineering Whoop 4.0». Independent review зафиксировал disposition Whoop относительно уже выбранной здесь архитектуры (MCP-агрегатор, см. §Архитектурные выводы).

### Disposition

| Путь | Что это | Решение |
|---|---|---|
| **Whoop через MCP-агрегатор (official API)** | Open Wearables / Delx Wellness уже поддерживают Whoop через **официальный Whoop Developer API** (OAuth 2.0, sanctioned, consent, webhooks, app-approval). Server-side агрегация → лаконичные сводки Claude | ✅ **Единственный одобренный путь.** Совпадает с Рекомендацией 1/2 выше. Whoop — просто один из провайдеров; ничего Whoop-специфичного строить не нужно |
| **`my-whoop` (reverse-engineered BLE)** | Local-first iOS (Swift) + сервер (Python FastAPI/TimescaleDB) ридер браслета WHOOP 4.0 по Bluetooth LE; unofficial (17 U.S.C. §1201(f)) | 🔴 **Отклонён** — причины ниже |

### Почему reverse-eng BLE (`my-whoop`) не подходит

1. **Неверная архитектура.** Per-device BLE-ридер противоречит выбранному aggregator-MCP паттерну (source-agnostic, server-side). Это приложение (iOS + FastAPI), а не MCP-источник; prompt-скилл его не запускает.
2. **Хрупкость.** Reverse-eng BLE-протокол ломается на firmware-апдейтах; поддерживается только WHOOP 4.0.
3. **Legal / ToS.** Unofficial; в `DISCLAIMER.md` явный takedown-контакт. Официальный API — sanctioned, с consent.
4. **Не валидировано.** Сам `my-whoop` дисклеймит «не мед.устройство, клинически не валидировано, не мед.совет» (это верно и для официальных метрик Whoop — любой wearable-путь обязан остаться advisory, не диагностикой; ср. SKILL.master §Safety).

### Связь с CRI-моделью и плечо

- Whoop recovery/HRV/sleep/strain питают **только objective-компонент (40%)** Composite Readiness Index (`composite_readiness_model.md` §5). Research-тезис: биометрия плохо коррелирует с самочувствием (r = 0.21–0.58), поэтому 60% — субъективный self-report, которому **трекер не нужен**. ⇒ Вкладываться в точный Whoop-пайплайн = **низкое плечо**.
- **Дешёвый do-now слой** (без MCP/OAuth/инфраструктуры): manual import recovery/HRV пользователем → BACKLOG-кандидат «Wearable readiness — manual import (Variant C)» (RICE 26.3, gated к v0.17.x). Валидирует спрос до постройки агрегатор-MCP.
- **Автоматизация** (aggregator-MCP) — это существующие BACKLOG-items `Google Health MCP integration research` + `Wearable Energy Integration` (v0.17.x). Whoop входит туда «бесплатно» как поддерживаемый провайдер официального API.
