...
Covers: adhd_mode.md, time_structure_unemployed.md, elder_homebound_mode.md,
...
class TestAdhdMode:
    """Validate references/adhd_mode.md"""

    def test_file_exists_and_size(self):
        path = PROJECT_ROOT / "references" / "adhd_mode.md"
        assert path.exists()
        lines = path.read_text(encoding="utf-8").splitlines()
        assert len(lines) <= 120

    def test_opt_in_framing(self):
        content = (PROJECT_ROOT / "references" / "adhd_mode.md").read_text(encoding="utf-8").lower()
    ...
    def test_no_medical_advice(self):
        lines = (PROJECT_ROOT / "references" / "adhd_mode.md").read_text(encoding="utf-8").lower().splitlines()
...
class TestTimeStructureUnemployed:
    """Validate references/time_structure_unemployed.md"""

    def test_file_exists_and_size(self):
        path = PROJECT_ROOT / "references" / "time_structure_unemployed.md"
        assert path.exists()
        lines = path.read_text(encoding="utf-8").splitlines()
        assert len(lines) <= 120

    def test_daily_template_present(self):
        content = (PROJECT_ROOT / "references" / "time_structure_unemployed.md").read_text(encoding="utf-8").lower()
...
class TestElderHomebound:
    """Validate references/elder_homebound_mode.md"""

    def test_file_exists_and_size(self):
        path = PROJECT_ROOT / "references" / "elder_homebound_mode.md"
        assert path.exists()
        lines = path.read_text(encoding="utf-8").splitlines()
        assert len(lines) <= 120

    def test_solo_aging_normalization(self):
        content = (PROJECT_ROOT / "references" / "elder_homebound_mode.md").read_text(encoding="utf-8").lower()
    ...
    def test_no_productivity_imposition(self):
        lines = (PROJECT_ROOT / "references" / "elder_homebound_mode.md").read_text(encoding="utf-8").lower().splitlines()
...
class TestPlanningFrictionAudit:
    """Validate references/planning_friction_audit.md"""

    def test_file_exists_and_size(self):
        path = PROJECT_ROOT / "references" / "planning_friction_audit.md"
        assert path.exists()
        lines = path.read_text(encoding="utf-8").splitlines()
        assert len(lines) <= 120

    def test_friction_detection_questions(self):
        content = (PROJECT_ROOT / "references" / "planning_friction_audit.md").read_text(encoding="utf-8").lower()
    ...
    def test_no_shaming(self):
        lines = (PROJECT_ROOT / "references" / "planning_friction_audit.md").read_text(encoding="utf-8").lower().splitlines()
        for i, line in enumerate(lines):
    ...
    def test_science_references(self):
        content = (PROJECT_ROOT / "references" / "planning_friction_audit.md").read_text(encoding="utf-8")
...
class TestPersonaHooks:
    ...
    def test_adhd_reference_loaded(self, platforms):
        for name, content in platforms.items():
            assert "adhd_mode.md" in content or "references/adhd" in content
    ...
    def test_unemployed_reference_loaded(self, platforms):
        for name, content in platforms.items():
            assert "time_structure_unemployed.md" in content or "references/time_structure" in content
    ...
    def test_elder_homebound_reference_loaded(self, platforms):
        for name, content in platforms.items():
            assert "elder_homebound_mode.md" in content or "references/elder_homebound" in content
...
class TestPlatformIntegrationV14:
    ...
    def test_all_four_references_in_master(self):
        content = (PROJECT_ROOT / "SKILL.master.md").read_text(encoding="utf-8")
        assert "adhd_mode.md" in content
        assert "time_structure_unemployed.md" in content
        assert "elder_homebound_mode.md" in content
        assert "planning_friction_audit.md" in content
