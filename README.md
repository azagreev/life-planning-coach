...
> **Версия:** 1.4.4 | [Релизы](https://github.com/azagreev/life-planning-coach/releases) | [CHANGELOG](CHANGELOG.md)
...
| **Хранение** | Файлы | Память внутри чата | Wiki в Google Drive + Google Calendar + AI-память (по желанию) |
...
- **Колесо жизненного баланса** (11 сфер, включая духовность и ценности)
...
¹ MCP-коннекторы (Google Calendar + Google Drive) подтверждены на **тарифе Claude Max** (проверка проведена 2026-05-26 — см. [docs/research/mcp_poc_log.md](docs/research/mcp_poc_log.md)). Тарифы Pro/Team не проверялись. На Free, скорее всего, MCP недоступен — запасной вариант — **текстовый режим коуча** (план без интеграций).
...
Сторонние маркетплейсы (как этот) **не авто-обновляются по умолчанию** — авто-pull на старте сессии включён только для официального маркетплейса Anthropic ([Claude Code #26744](https://github.com/anthropics/claude-code/issues/26744)). Поэтому после нового релиза кнопка Update может оставаться неактивной, пока обновление не подтянуть. Версии в манифесте проставлены корректно — нужно лишь включить (или разово запустить) обновление.

**✅ Рекомендуется — включить auto-update один раз** (дальше плагин обновляется сам на старте сессии):
- **Claude Code (CLI/desktop):** `/plugin` → **Marketplaces** → `life-planning-coach` → включить **auto-update**. Пока это единственный способ включить авто-обновление стороннего маркетплейса — только через `/plugin` TUI ([#51350](https://github.com/anthropics/claude-code/issues/51350)).
- **Cowork:** включи тумблер **auto-update** на странице плагина, если он показан.

**Разовое обновление вручную:**
- **Cowork (GUI):** Customize → `life-planning-coach` → **удали и добавь заново** (`+` → Add marketplace from GitHub → `azagreev/life-planning-coach`) — форсит свежий клон.
- **Claude Code (CLI):** `/plugin marketplace update life-planning-coach` → обнови плагин → `/reload-plugins` (или перезапусти сессию). Репозиторий публичный, авторизация не нужна.
...
3. Версия всё ещё старая → **удали маркетплейс и добавь заново** (самый надёжный способ для сторонних маркетплейсов).
...
1. Скачайте `life-planning-coach-v1.4.4.zip` из раздела [Релизы](https://github.com/azagreev/life-planning-coach/releases)
...
1. Скачайте `life-planning-coach-v1.4.4-grok.md`
...
2. Создайте агента → вставьте содержимое `life-planning-coach-v1.4.4-kimi.md` в системный промпт
